...
void proceed(Solution05 * handle) {
	...
	for(i = 0; i<NUM_TIMERS; i++) {
		if(timers[i].handle == handle && timers[i].evid != 0 && time >= timers[i].end_time) {
			...
			solution05_raiseTimeEvent(handle, evid);
		}
	}
}

/*! Function to retrieve the current timestamp, ideally in milliseconds. */
unsigned long get_current_timestamp(Solution05* handle) {
...
void set_current_timestamp(Solution05* handle, unsigned long timestamp) {
	unsigned long ms = get_ms();
	timestamp_offset = ms - timestamp;
}

void solution05_setTimer(Solution05* handle, const sc_eventid evid, const sc_integer time_ms, const sc_boolean periodic) {
	unsigned long time = get_current_timestamp(handle);
